import { MAX_ATTACHED, STATE } from '../state.js';
import { baseLeft, dropTargetOptions, reloadBtn, table } from '../settings.js';
...
export const dragDropHandler = (evt) => {
	...
	const mouseMoveHandler = (dropEvt) => {
		...
		const targetClass = STATE.dropTargets.filter(
			(item) =>
				Math.abs(currentLeft + baseLeft - dropTargetOptions[item].left) < card.clientWidth &&
