...
const dragDropHandler = (evt) => {
  ...
  const mouseMoveHandler = (dropEvt) => {
    ...
    const targetClass = STATE.dropTargets.filter((item) => {
      return (Math.abs(currentLeft + baseLeft - dropTargetOptions[item].left) < card.clientWidth
    ...
    if (targetClass) {
      ...
      if (isCorner > -1) {
        ...
        if (dataAccept[0] === `any` && dataAccept[1] === `13` && cornerSuits.indexOf(dataCard[0]) > -1) {
          isTrueValue = false;
        }
      }
      ...
      if (isTrueSuit && isTrueValue) {
        ...
        let newAcceptValue = cardValue !== 13 ? cardValue + 1 : 1;
        if (isCorner > -1) {
          newAcceptValue = cardValue !== 1 ? cardValue - 1 : 13;
        }
  ...
  const mouseUpHandler = (upEvt) => {
    ...
    if (isTrueAccept) {
      ...
      if (card.className.indexOf(`corner`) > -1) {
        ...
        if (cornerSuits.indexOf(dataCard[0]) === -1) {
          cornerSuits.push(dataCard[0]);
        }
      } else {
        card.classList.add(`card--draggable`);
      }
      if (numberOfAttached === 104) {
        showWinMessage();
      }
    ...
    if (table.querySelectorAll(`.card--control`).length < 8) {
